...
- Redirect contract: one-hop `302` with query preservation
...
The optional First Build Brief form is hidden and inactive. The current site is a static Cloudflare Pages direct upload with no established server endpoint or rate-limit binding, and Encharge sender approval and double opt-in are not ready. No endpoint, form capture, Encharge secret, success promise, Meta Pixel, ad launch, or spend was added.
